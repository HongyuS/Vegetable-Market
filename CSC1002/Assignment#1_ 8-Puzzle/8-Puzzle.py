...
def random_board(p_board):                 # function to generate a new puzzle
    ...
    inversion = 0
    for i in range(len(list1)):            # calculate the total_inversion
    ...
    if inversion % 2 == 0:                 # check solvability:
        while list1 != []:                 # if total_inversion is odd:
            p_board.append(list1[:3])      # ____the puzzle is unsolvable
...
def judge_direction(p_zero):               # function to judge valid direction
    ...
    if p_zero[1] == 2:                     # take out invalid item
...
def operate(                               # function to move a number
        p_board,
        p_valid_direction,                 # valid direction hint
        p_zero):                           # the position of zero
    while True:
        direction = input(
            'Input sliding direction (' + p_valid_direction + ') >'
        )
        if ((direction == LEFT and p_zero[1] == 2)
                or (direction == RIGHT and p_zero[1] == 0)
                or (direction == UP and p_zero[0] == 2)
                or (direction == DOWN and p_zero[0] == 0)
                or not (direction in [LEFT, RIGHT, UP, DOWN])):
            print('Invalid move! Please try again!')
        else:
            if direction == LEFT:
                p_board[p_zero[0]][p_zero[1]],\
                    p_board[p_zero[0]][p_zero[1]+1]\
